...
router.post("/inventory/addCar", authentication, async (req: RequestWithUser, res) => {
  const admin = await prisma.admin.findUnique({
...
router.put("/inventory/:id", authentication, async (req: RequestWithUser, res) => {
  const id = parseInt(req.params.id);
...
router.delete("/inventory/:id", authentication, async (req: RequestWithUser, res) => {
  const id = parseInt(req.params.id);
  const admin = await prisma.admin.findUnique({
    where: { adminMail: req.user.email },
  });
