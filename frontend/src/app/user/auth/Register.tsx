   ...
   const [user, setUser] = useState({
    ...
    const handleAddUser = async (e: React.MouseEvent<HTMLButtonElement>) => {
      ...
      if (user.email && user.name && user.password && !loading) {
         setLoading(true);
         const response = await axios.post("https://calm-gold-rabbit-gown.cyclic.app/user/register", user)
      ...
      <>
         ...
         <div className='m-auto w-[50%] flex flex-col gap-4'>
            <motion.div className='flex flex-col gap-1 mt-[4vw]' variants={variants} initial="hidden" animate="enter" transition={{ ease: "easeOut", duration: 1 }}>
               <div>
                  <p className='text-[#333333] text-4xl font-semibold tracking-tighter'>Register</p>
                  <span className='text-6xl font-semibold gradient-text tracking-tight'>Welcome Turquioaz!</span>
               </div>
