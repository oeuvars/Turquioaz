...
const forgotPassword = async (req, res) => {
    ...
    if (user && user.is_verified === true) {
        ...
        const otpUpdated = await db_config_1.default.user.update({ where: { email: email }, data: { otp: randomOTP } });
        const token = jsonwebtoken_1.default.sign({ email: email }, process.env.hiddenKey, { expiresIn: '1d' });
        ...
        (0, sendMail_1.sendMail)(email, "Forgot Password?", content);
        if (otpUpdated) {
