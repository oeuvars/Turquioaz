...
const rentCar = async (req, res) => {
    ...
    if (car) {
        ...
        if (user) {
            ...
            const rentedCar = await db_config_1.default.rentedCar.create({ data: { carId: carID, rentedtoId: userID, startDate: startDate, endDate: endDate, status: status } });
            const rentedCarID = rentedCar.id;
            const token = jsonwebtoken_1.default.sign({ id: rentedCarID }, process.env.hiddenKey, { expiresIn: "24h" });
